...
function parseSize(sizeText) {
		...
		return {
			...
			unit: match[2].toLowerCase().replace(/\.$/, "").trim()
		};
	}
	function formatPricePerUnit(price, unit) {
		const decimals = price < .01 ? 4 : price < 1 ? 3 : 2;
...
get targetWindow() {
		...
		async addPricePerUnitOnProductPage() {
			...
			this.createObserver(container, priceContainer, (el) => priceContainer.appendChild(el));
			const productInfo = this.extractProductInfo(container);
			if (!productInfo?.pricePerUnit) return;
			const el = createPricePerUnitElement(formatPricePerUnit(productInfo.pricePerUnit, productInfo.unit));
		...
		async waitForMainThenAddSort() {
			...
			while (!document.querySelector(readySelector) && Date.now() < deadline) await new Promise((r) => setTimeout(r, interval));
		...
		async addSortByUnitPriceOnListingPage() {
			...
			if (!desktopContainer && !mobileContainer) {
				...
				else document.body.appendChild(this.createSortButtonsFragment());
		...
		interceptPlpFetch() {
			...
			win.fetch = (input, init) => {
				...
				if (typeof url === "string" && url.includes("/plp/api/search")) return orig.call(win, input, init).then(async (res) => {
					const clone = res.clone();
					try {
						const json = await clone.json();
						if (json?.products && Array.isArray(json.products)) this.plpProducts = json.products.map(ChewyPricePerUnit.mapPlpProduct);
					} catch (e) {
		...
		async sortListingByUnitPrice(order) {
			...
			const cards = Array.from(gridContainer.querySelectorAll(ChewyPricePerUnit.PRODUCT_CARD_SELECTOR));
			...
			for (const card of cards) gridContainer.appendChild(card);
	...
	var currentHandler = [{
		...
		handler: class PetSmartPricePerUnit extends BaseSiteHandler {
			...
			getSizeText(container) {
				...
				for (const key of keys) if (key.textContent?.toLowerCase().includes("size")) {
			...
			extractProductInfo(element) {
				...
				const priceMatch = ((priceEl.querySelector(".sparky-c-price--sale")?.textContent || priceEl.textContent)?.trim() || "").match(/\$[\d,]+(?:\.\d{2})?/);
				const price = priceMatch ? parseFloat(priceMatch[0].replace(/[$,]/g, "")) : NaN;
				if (!Number.isFinite(price)) return null;
...
parseSizeFromTitle(title) {
				...
				let last = null;
				let match;
				while ((match = regex.exec(title)) !== null) last = match;
			...
			addPpuToCard(card) {
				...
				const pricePerUnit = price / sizeInfo.quantity;
				priceGroup.appendChild(createPricePerUnitElement(formatPricePerUnit(pricePerUnit, sizeInfo.unit)));
			}
		}
	}, {
	...
	if (currentHandler) new currentHandler.handler().initialize().catch(console.error);
})();