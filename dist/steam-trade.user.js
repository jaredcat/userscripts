...
`;
	...
	var CookieManager = {
		...
		restoreInventoryCookie(cookieValue) {
			...
			expiry.setTime(expiry.getTime() + CONFIG.COOKIE.EXPIRY_DAYS * 24 * 60 * 60 * 1e3);
			document.cookie = `strTradeLastInventoryContext=${cookieValue}; expires=${expiry.toUTCString()}; path=/tradeoffer/`;
		}
	};
	var InventoryValidator = class {
		static validateInventories(yourInventory, theirInventory) {
			if (!Object.keys(yourInventory).length || !Object.keys(theirInventory).length) throw new Error("Invalid inventory state");
		}
	...
	var TradeHandler = class {
		...
		validateTrade() {
			...
			if (this.users.length < 2) throw new Error("Invalid users array");
			...
			if (!validCards.yours.length) throw new Error("No valid matching cards found in both inventories");
			return true;
		}
		findValidCards(yourInventory, theirInventory, yourCards, theirCards) {
			...
			yourCards.forEach((classId) => {
				if (Object.values(yourInventory).some((item) => item.classid === classId)) validYourCards.add(classId);
			});
			theirCards.forEach((classId) => {
				if (Object.values(theirInventory).some((item) => item.classid === classId)) validTheirCards.add(classId);
			});
		...
		processCards() {
			...
			for (let i = 0; i < 2; i++) {
				...
				if (!user) throw new Error(`User at index ${i} is undefined`);
				const inventory = user.rgContexts[CONFIG.STEAM.APP_ID]?.[CONFIG.STEAM.CONTEXT_ID]?.inventory;
				if (!inventory) throw new Error(`Inventory for user ${i} is undefined`);
				inventory.BuildInventoryDisplayElements();
				const cards = this.cards[i];
				if (!cards) throw new Error(`Cards array at index ${i} is undefined`);
				const cardMapping = this.createCardMapping(inventory.rgInventory || {}, cards);
				const cardTypeArray = cardTypes[i];
				if (!cardTypeArray) throw new Error(`Card types array at index ${i} is undefined`);
		...
		createCardMapping(inventory, requestedCards) {
			...
			if (this.settings.ORDER === "SORT") Object.values(mapping).forEach((cards) => {
				if (cards) cards.sort((a, b) => parseInt(b.id) - parseInt(a.id));
			});
			return mapping;
		}
	...
	var TradeSplitter = class {
		...
		openTradeWindow(baseUrl, youParams, themParams, startIndex) {
			...
			newUrl.search = "";
			const youChunk = youParams.slice(startIndex, startIndex + CONFIG.TRADE.CHUNK_SIZE);
			const themChunk = themParams.slice(startIndex, startIndex + CONFIG.TRADE.CHUNK_SIZE);
	...
	var SettingsManager = class SettingsManager {
		...
		static async loadSettings() {
			...
			const autoSend = await _GM.getValue("AUTO_SEND", CONFIG.DEFAULT_SETTINGS.AUTO_SEND);
			if (typeof autoSend === "boolean") settings.AUTO_SEND = autoSend;
			const doAfterTrade = await _GM.getValue("DO_AFTER_TRADE", CONFIG.DEFAULT_SETTINGS.DO_AFTER_TRADE);
			if (this.isValidDoAfterTrade(doAfterTrade)) settings.DO_AFTER_TRADE = doAfterTrade;
			const order = await _GM.getValue("ORDER", CONFIG.DEFAULT_SETTINGS.ORDER);
			if (this.isValidCardOrder(order)) settings.ORDER = order;
			const sideBySide = await _GM.getValue("SIDE_BY_SIDE", CONFIG.DEFAULT_SETTINGS.SIDE_BY_SIDE);
			if (typeof sideBySide === "boolean") settings.SIDE_BY_SIDE = sideBySide;
			return settings;
		}
		static async saveSettings() {
			const message = document.getElementById("trade-message")?.value?.trim() || CONFIG.DEFAULT_SETTINGS.MESSAGE;
			...
			const doAfterTrade = document.getElementById("after-trade")?.value;
			if (!this.isValidDoAfterTrade(doAfterTrade)) throw new Error(`Invalid DO_AFTER_TRADE value: ${doAfterTrade}`);
			const order = document.getElementById("cards-order")?.value;
			if (!this.isValidCardOrder(order)) throw new Error(`Invalid ORDER value: ${order}`);
			const settings = {
				MESSAGE: message,
				DO_AFTER_TRADE: doAfterTrade,
				ORDER: order,
				AUTO_SEND: document.getElementById("auto-send")?.checked ?? CONFIG.DEFAULT_SETTINGS.AUTO_SEND,
				SIDE_BY_SIDE: document.getElementById("side-by-side")?.checked ?? CONFIG.DEFAULT_SETTINGS.SIDE_BY_SIDE
			};
    ...
    `;
        ...
        ${[
				...
				},
				{
					...
					body: this.createCheckboxCard("auto-send", "Auto-send trade offer", "Makes it possible for the script to automatically send trade offers without any action.", settings.AUTO_SEND)
				},
				{
					title: "Side-by-side trade view",
					body: this.createCheckboxCard("side-by-side", "Side-by-side trade view", "Changes the steam trade window so that the trade columns are side-by-side.", settings.SIDE_BY_SIDE)
				}
			].map((card) => this.createCard(card.title, card.body)).join("")}
	...
	async function handleSteamTrade(settings) {
		...
		try {
			...
			const tradeOfferNote = document.getElementById("trade_offer_note");
	...
	async function init() {
		...
		if (window.location.host === "steamcommunity.com") {
			if (window.location.pathname.startsWith("/tradeoffer/new/")) await new StyleManager().initialize();
			if (window.location.search.includes("source=stm")) await handleSteamTrade(settings);
		} else if (CONFIG.WEBSITE_HOSTS.includes(window.location.host)) {
			new TradeSplitter();
			document.addEventListener("turbo:load", () => {
				if (window.location.pathname === "/userscript") SettingsManager.prepareSettings(settings);
			});
		}
	}
